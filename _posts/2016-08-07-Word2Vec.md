...
$$\log(p(w_o \vert w_c)) =  {\hat{\textbf{v}}_j}^{T} {\textbf{v}}_c -  \log(  {\sum_{t=1}^{N_{\textbf{V}}}  \exp({\hat{\textbf{v}}_t}^{T} {\textbf{v}}_c)} ) = -E $$. 

Here, $$E = - \log(p(w_o \vert w_c)) = - {\hat{\textbf{v}}_j}^{T} {\textbf{v}}_c + \log(  {\sum_{t=1}^{N_{\textbf{V}}}  \exp({\hat{\textbf{v}}_t}^{T} {\textbf{v}}_c)} ) $$ is the loss function (we want to minimize $$E$$ ), and $$o$$ is the index of the correct outside word in the output layer.
