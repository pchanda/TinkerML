...
Likelihood: $$\log(p(w_o \vert w_c)) =  {\hat{\textbf{v}}_o}^{T} {\textbf{v}}_c -  \log(  {\sum_{t=1}^{N_{\textbf{V}}}  \exp({\hat{\textbf{v}}_t}^{T} {\textbf{v}}_c)} ) = -E $$. $$\tag{1}$$
...
Intuitively, update $$\hat{\textbf{v}}_j$$ if $$p(w_j \vert w_c) \gt 0$$ (over-estimation, subtract $$\eta$$ times the difference from $$\hat{\textbf{v}}_j$$.
...
Intuitively, update $$\hat{\textbf{v}}_o$$ if $$p(w_o \vert w_c) \lt 1$$ (under-estimation, add $$\eta$$ times the difference to $$\hat{\textbf{v}}_o$$.
