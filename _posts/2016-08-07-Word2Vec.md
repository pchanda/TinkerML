...
When $$j=o$$, the correct index of the outside word, $$ y_j = y_o = p( w_o \vert w_c)$$. Now we want to maximize the probability for the correct outside word prediction, i.e. $$p(w_o \vert w_c)$$, or, the log of it : $$log(p(w_o \vert w_c)) =  {\hat{\textbf{v}}_j}^{T} {\textbf{v}}_c -  log (  {\sum_{t=1}^{N_{\textbf{V}}}  \exp({\hat{\textbf{v}}_t}^{T} {\textbf{v}}_c)} )$$. 
