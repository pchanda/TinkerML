...
So for all indices $$j$$ which does not correspond to the correct outside woth $$w_o$$, using stochastic gradient descent, the weight updating equation for hidden-to-output weight vector $$\hat{\textbf{v}}_j$$ is, 

$$ {\hat{\textbf{v}}_j}^{(new)} = {\hat{\textbf{v}}_j}^{(old)} - \eta [p(w_j \vert w_c) - 0] {\textbf{v}}_c $$. 

And for the vector $$\hat{\textbf{v}}_o$$ for the correct output word $$w_o$$ is 
