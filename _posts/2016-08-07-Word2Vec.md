...
Likelihood: $$\log(p(w_o \vert w_c)) =  {\hat{\textbf{v}}_o}^{T} {\textbf{v}}_c -  \log(  {\sum_{t=1}^{N_{\textbf{V}}}  \exp({\hat{\textbf{v}}_t}^{T} {\textbf{v}}_c)} ) = -E $$. 
...
$$ {\hat{\textbf{v}}_j}^{(new)} = {\hat{\textbf{v}}_j}^{(old)} - \eta [p(w_j \vert w_c) - 0] {\textbf{v}}_c $$. Intuitively, update $$\hat{\textbf{v}}_j$$ if $$p(w_j \vert w_c) \gt 0$$ (over-estimation, subtract $$eta$$ times the difference from $$\hat{\textbf{v}}_j$$.

And for the vector $$\hat{\textbf{v}}_o$$ (for the correct output word $$w_o$$), the update is 

$$ {\hat{\textbf{v}}_o}^{(new)} = {\hat{\textbf{v}}_o}^{(old)} - \eta [p(w_o \vert w_c) - 1] {\textbf{v}}_c $$.  Intuitively, update $$\hat{\textbf{v}}_o$$ if $$p(w_o \vert w_c) \lt 1$$ (under-estimation, add $$eta$$ times the difference to $$\hat{\textbf{v}}_o$$.
