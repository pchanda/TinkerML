...
Using these weights, we can compute a score $$u_j$$ for each word in the vocabulary:  $$u_j = {\hat{\textbf{v}}_j}^{T} \textbf{h} = {\hat{\textbf{v}}_j}^{T} \textbf{v}_c$$. That is, using matrix multiplication, $$ \textbf{u} = \hat{\textbf{W}}^T h$$, $$\textbf{u}$$ is a $$ N_{\textbf{V}} \times 1$$ column vector.

$$u_j = {\hat{\textbf{v}}_j}^{T} \textbf{h} $$.
